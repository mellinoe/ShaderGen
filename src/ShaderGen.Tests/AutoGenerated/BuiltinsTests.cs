...
namespace ShaderGen.Tests.AutoGenerated
{
    public class BuiltinsTests
    {
        ...
        private void TestBuiltins()
        {
            ...
            {
                testSets = new TestSets(toolChains, compilation, mappings, TestLoops);
            }
             ...
             */
            IReadOnlyDictionary<MethodMap, IReadOnlyList<Failure>> failures;
            using (new TestTimer(_output, "Analysing results for failures"))
            {
                failures = testSets.GetFailures();
            }
            ...
            foreach (KeyValuePair<MethodMap, IReadOnlyList<Failure>> kvp in failures)
            {
                MethodMap methodMap = kvp.Key;
                IReadOnlyList<Failure> methodFailures = kvp.Value;
